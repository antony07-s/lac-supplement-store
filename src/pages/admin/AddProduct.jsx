...
function AddProduct() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (cleanedVariants.some((variant) => !variant.packSize || !Number.isFinite(variant.price) || variant.price < 0 || !Number.isSafeInteger(variant.stock) || variant.stock < 0 || !Number.isFinite(variant.shippingWeightKg) || variant.shippingWeightKg <= 0)) {
      toast.error('Complete every variant with a pack size, price, stock, and shipping weight')
      return
    }
    setSaving(true)
    try {
    ...
    <AdminLayout title="Add Product" subtitle="Create a new product in your catalog" backTo="/admin/products">
      <div className="max-w-2xl bg-white border border-gray-200 rounded-2xl p-6 md:p-8">
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <label className="flex items-center gap-2 rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 text-sm font-medium text-stone-700"><input type="checkbox" name="bestSeller" checked={form.bestSeller} onChange={(event) => setForm((current) => ({ ...current, bestSeller: event.target.checked }))} /> Show in homepage Best Sellers</label>
